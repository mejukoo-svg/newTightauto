...
# 3단계: Mixpanel
print("="*60); print(f"3단계: Mixpanel 수집 (최근 {REFRESH_DAYS}일만)"); print("="*60)
YESTERDAY=TODAY-timedelta(days=1);mp_from=DATA_REFRESH_START.strftime('%Y-%m-%d');mp_to_yesterday=YESTERDAY.strftime('%Y-%m-%d');mp_to_today=TODAY.strftime('%Y-%m-%d')
total_days=(TODAY-DATA_REFRESH_START).days+1
print(f"  📅 Mixpanel 범위: {mp_from} ~ {mp_to_today} ({total_days}일)")
print(f"  ℹ️ 이전 날짜탭은 기존 스프레드시트 데이터 유지")
mp_raw=[]
...
print(f"\n  ✅ Mixpanel 수집 완료: 총 {len(mp_raw)}건 (최근 {REFRESH_DAYS}일)")
...
print(f"🔄 Mixpanel 갱신: 최근 {REFRESH_DAYS}일 ({DATA_REFRESH_START.strftime('%Y-%m-%d')} ~ 오늘)")
