# -*- coding: utf-8 -*-
# v3-ad 통합 코드 (★ v29 - 모든 금액 USD 달러 표시, KRW 환산 완전 제거):
#   - ★ v29: 환율 조회/변환 완전 제거 — Meta spend = USD 그대로, Mixpanel revenue = USD 그대로
#   - ★ v29: ₩ → $ 표기 전환, 헤더 "지출 금액 (KRW)" → "지출 금액 (USD)"
#   - ★ v29: money() 함수 달러 표시, 숫자 포맷 [$₩-412] → [$-en-US]
...
print("="*60)
print("🚀 v3-ad v29 (모든 금액 USD 달러 표시, KRW 환산 완전 제거)")
print("="*60)
...
print(f"💲 ★v29: 모든 금액 USD 달러 표시 (환율 변환 없음)")
...
def generate_date_tab_summary(rows, structure="new"):
    ...
    r_hdr[14]="지출 금액 (USD)";r_hdr[15]="구매 (메타)";r_hdr[16]="구매 (믹스패널)"
    ...
    r_val[14]=round(total_spend,2);r_val[15]=round(total_meta_purchase);r_val[16]=round(total_mp_purchase)
    r_val[17]=round(total_revenue,2);r_val[18]=round(total_profit,2);r_val[19]=round(total_roas,1);r_val[20]=total_cvr/100 if total_cvr else 0
...
if len(df)>0:
    ...
    print(f"  utm_term 필터 후 중복제거: {len(df_d)}건 | 매출합: ${df_d['revenue'].sum():,.2f}")
...
# ★ v29: 환율 조회 단계 완전 제거 — 모든 금액 USD 그대로 사용
print("="*60); print("4.5단계: ★v29 환율 조회 불필요 — 모든 금액 USD 그대로 사용"); print("="*60)
print("  💲 Meta spend = USD, Mixpanel revenue = USD → 변환 없음")
print()

# =========================================================
# ★ v29: 5단계 - Meta + Mixpanel 병합 (USD 그대로, 환율 변환 없음)
# =========================================================
print("="*60); print("5단계: Meta + Mixpanel 병합 (★v29: 모든 금액 USD)"); print("="*60)
...
for dk in sorted(meta_date_data.keys(),key=lambda x:meta_date_data[x][0]['date_obj'] if meta_date_data[x] else datetime.min,reverse=True):
    ...
    for mr in rows:
        ...
        # ★ v29: USD 그대로 — 환율 곱셈 제거
        sp=mr['spend'];impr=mr['impressions'];reach=mr['reach'];uc=mr['unique_clicks'];results=mr['results']
        ...
        # ★ v29: 예산도 USD 그대로 (센트→달러 변환만)
        budget_raw = adset_budget_map.get(asid, 0)
        budget_val = round(budget_raw / 100, 2) if budget_raw and budget_raw > 0 else ""
    ...
    try:
            ...
            if asid not in new_row_by_asid:
                if asid in mp_data:
                    ...
                    batch_updates.append({'range':f'{mp_col_letter}{row_num}:{cvr_col_letter}{row_num}',
                        'values':[[int(mpc) if mpc>0 else "",round(revenue,2) if revenue>0 else "",round(profit,2) if spend>0 else "",round(roas_val,1) if spend>0 and revenue>0 else "",round(cvr_val,2) if unique_clicks>0 and mpc>0 else ""]]})
                continue
        ...
        for asid, row_idx in adset_id_row_map.items():
            if asid in adset_budget_map and adset_budget_map[asid] > 0:
                row_num = row_idx + 1; budget_usd = round(adset_budget_map[asid] / 100, 2)
                budget_updates.append({'range': f'{budget_col_letter}{row_num}', 'values': [[budget_usd]]})
        if budget_updates:
...
# ★ v29: mp_value_map 그대로 사용 (환율 변환 없음)
mp_value_map_usd = mp_value_map  # USD 그대로
...
print(f"💲 ★v29: 모든 금액 USD 달러 표시 (환율 변환 완전 제거)")
print(f"📦 제품명: ★v27 동적 추출 → {len(products)}개 감지")
