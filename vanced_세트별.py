...
사용법:
  1) .env 파일 또는 환경변수 설정 (README 참고)
  2) python main.py
  3) 또는 GitHub Actions가 매일 자동 실행
"""
...
# 필수 환경변수 — ★ 수정: 다른 스크립트와 Secret 이름 통일
META_ACCESS_TOKEN    = os.environ.get('META_TOKEN_1', '')
META_AD_ACCOUNT_ID   = os.environ.get('META_AD_ACCOUNT_ID', 'act_25183853061243175')
...
MIXPANEL_EVENT_NAME  = os.environ.get('MIXPANEL_EVENT_NAME', '결제완료')

SPREADSHEET_URL      = os.environ.get('SPREADSHEET_URL', '')
GCP_SA_KEY_JSON      = os.environ.get('GCP_SERVICE_ACCOUNT_KEY', '')

FROM_DATE = os.environ.get('FROM_DATE', '2025-11-09')
...
def extract_action(al, types):
    if not al: return 0
    for a in al:
        if a.get('action_type') in types: return float(a.get('value', 0))
    return 0

def extract_outbound(dl):
    if not dl: return 0
    for a in dl:
        if a.get('action_type') in ['outbound_click','link_click']: return float(a.get('value',0))
    return float(dl[0].get('value',0)) if len(dl)==1 else 0

def fmt(v): return '0' if not v else f"{int(round(v)):,}"
def fmtp(v): return '0.00%' if not v else f"{v:.2f}%"
...
def fetch_meta():
    ...
    print("="*80)
    print(f"\n📡 [1] Meta Ads API: adset 일간 데이터 ({FROM_DATE} ~ {TO_DATE})...")
    ...
    while True:
        ...
        if next_url:
            ...
            resp = requests.get(f"{META_BASE_URL}/{META_AD_ACCOUNT_ID}/insights", params={
        ...
        if resp.status_code != 200:
            print(f"   ❌ {resp.status_code}: {resp.text[:300]}"); break
        data = resp.json()
        rows = data.get('data', [])
        meta_raw.extend(rows)
        print(f"   페이지 {page}: +{len(rows)}건 (누적 {len(meta_raw)}건)")
        next_url = data.get('paging', {}).get('next')
        if not next_url or not rows: break
        time.sleep(0.5)
    ...
    for row in meta_raw:
        ...
        aid, an, cn = row['adset_id'], row.get('adset_name',''), row.get('campaign_name','')
        sp, imp = float(row.get('spend',0)), float(row.get('impressions',0))
        pr_list = row.get('purchase_roas', [])
        meta_data[(dk, aid)] = {
            'date': dk, 'campaign_name': cn, 'adset_name': an, 'adset_id': aid,
            'product': adset_to_product(an),
            'spend': sp, 'cpm': float(row.get('cpm',0)),
            'reach': float(row.get('reach',0)), 'impressions': imp,
        ...
        if aid not in all_adsets:
            all_adsets[aid] = {'campaign_name': cn, 'adset_name': an, 'product': adset_to_product(an)}
...
# Part 2: Mixpanel
# =============================================================================
def fetch_mixpanel(all_adsets):
    print(f"\n📡 [2] Mixpanel API: 결제완료 이벤트 조회...")
    resp = requests.get("https://data.mixpanel.com/api/2.0/export",
    ...
    if resp.status_code != 200:
        print(f"❌ Mixpanel 오류: {resp.status_code}")
    ...
    for line in lines:
        try:
            ...
            raw_amount, raw_value = props.get('amount'), props.get('value')
            amount = 0.0
            ...
            if amount <= 0 and raw_value is not None and str(raw_value).strip() not in ['','None','nan','null']:
                try: amount = float(raw_value)
                except: pass
            events_parsed.append({
                'dk': dk, 'ts': ts, 'utm_term': utm_term,
                'amount': amount, 'distinct_id': props.get('distinct_id',''),
    ...
    for e in deduped:
        utm = e['utm_term']
        if not utm: unmatched_events += 1; continue
...
def build_master(meta_data, mp_matched):
    ...
    for key, meta in sorted(meta_data.items(), key=lambda x: (date_key_to_dt(x[0][0]), x[0][1]), reverse=True):
        ...
        master_rows.append({**meta, 'purchases_mp': mp['count'], 'revenue_mp': mp['revenue'],
                            'profit': profit, 'roas_calc': roas, 'cvr': cvr})
    ...
    for date in all_dates[:5]:
        ...
        print(f"   {date}: 지출=₩{ts:,.0f} | 결과(MP)={tp}건 | MP매출=₩{tr:,.0f} | ROAS={tr/ts*100:.0f}%")
...
def calc_derived(master_rows, all_dates):
    ...
    for date in all_dates:
        rows = [r for r in master_rows if r['date'] == date]
        sp = sum(r['spend'] for r in rows); rv = sum(r['revenue_mp'] for r in rows)
        imp = sum(r['impressions'] for r in rows); uc = sum(r['unique_ob_clicks'] for r in rows)
        daily_total[date] = {
            'spend': sp, 'revenue': rv, 'profit': rv-sp, 'roas': rv/sp*100 if sp>0 else 0,
            ...
            'impressions': imp, 'cpm': sp/imp*1000 if imp>0 else 0,
        }
    daily_product = {}
    for date in all_dates:
        for p in PRODUCTS:
            rows = [r for r in master_rows if r['date']==date and r['product']==p]
            if not rows: daily_product[(date,p)] = {'spend':0,'revenue':0,'profit':0,'roas':0}; continue
            sp = sum(r['spend'] for r in rows); rv = sum(r['revenue_mp'] for r in rows)
            daily_product[(date,p)] = {'spend':sp,'revenue':rv,'profit':rv-sp,'roas':rv/sp*100 if sp>0 else 0}
    adset_daily = defaultdict(dict)
    for r in master_rows: adset_daily[r['adset_id']][r['date']] = r
    return daily_total, daily_product, adset_daily

# =============================================================================
# Part 5: Google Sheets 업데이트
# =============================================================================
def update_sheets(master_rows, all_dates, all_adsets, daily_total, daily_product, adset_daily):
    ...
    if ms: ws.update(range_name='A2', values=ms)
    print(f"   ✅ {len(ms)}행"); time.sleep(2)
    ...
    for row in existing[1:]:
        if row[0]=='종합': tao.append(('종합',None,None))
        elif len(row)>2 and row[2]: tao.append((row[0],row[1],row[2]))
    eids = {t[2] for t in tao if t[2]}
    for aid, info in all_adsets.items():
        if aid not in eids: tao.append((info['campaign_name'],info['adset_name'],aid))
    if not any(t[0]=='종합' for t in tao): tao.insert(0,('종합',None,None))
    ...
    for cn,an,aid in tao:
        if cn=='종합':
            row=['종합','','']
            r7=dc[:7]; ts=sum(daily_total.get(d,{}).get('spend',0) for d in r7); tr=sum(daily_total.get(d,{}).get('revenue',0) for d in r7)
            row.append(trend_cell(ts,tr,tr-ts,tr/ts*100 if ts>0 else 0))
            for date in dc:
                dt=daily_total.get(date,{}); row.append(trend_cell(dt.get('spend',0),dt.get('revenue',0),dt.get('profit',0),dt.get('roas',0)))
        else:
            row=[cn or '',an or '',aid or '']
            r7=dc[:7]; ts=sum(adset_daily.get(aid,{}).get(d,{}).get('spend',0) for d in r7); tr=sum(adset_daily.get(aid,{}).get(d,{}).get('revenue_mp',0) for d in r7)
            row.append(trend_cell(ts,tr,tr-ts,tr/ts*100 if ts>0 else 0))
            for date in dc:
                ad=adset_daily.get(aid,{}).get(date)
                row.append(trend_cell(ad['spend'],ad['revenue_mp'],ad['profit'],ad['roas_calc']*100) if ad else '')
        td.append(row)
    ws_t.clear(); ws_t.update(range_name='A1',values=td)
    ...
    for cn,an,aid in tao:
        if cn=='종합':
            row=['종합','','']
            r7=dc[:7]; ts=sum(daily_total.get(d,{}).get('spend',0) for d in r7); tr=sum(daily_total.get(d,{}).get('revenue',0) for d in r7); ti=sum(daily_total.get(d,{}).get('impressions',0) for d in r7)
            row.append(change_cell(tr/ts*100 if ts>0 else 0,0,ts,ts/ti*1000 if ti>0 else 0))
            for i,date in enumerate(dc):
                dt=daily_total.get(date,{}); rt=dt.get('roas',0)
            ...
            row=[cn or '',an or '',aid or '']
            r7=dc[:7]; ts=sum(adset_daily.get(aid,{}).get(d,{}).get('spend',0) for d in r7); tr=sum(adset_daily.get(aid,{}).get(d,{}).get('revenue_mp',0) for d in r7); ti=sum(adset_daily.get(aid,{}).get(d,{}).get('impressions',0) for d in r7)
            row.append(change_cell(tr/ts*100 if ts>0 else 0,0,ts,ts/ti*1000 if ti>0 else 0))
            for i,date in enumerate(dc):
                ad=adset_daily.get(aid,{}).get(date)
                if ad:
                    rt=ad['roas_calc']*100; pa=adset_daily.get(aid,{}).get(dc[i+1]) if i+1<len(dc) else None
                    pr=pa['roas_calc']*100 if pa and pa['roas_calc']>0 else 0
                    row.append(change_cell(rt,((rt-pr)/pr*100) if pr>0 else 0,ad['spend'],ad['cpm']))
                else: row.append('')
        cd.append(row)
    ws_c.clear(); ws_c.update(range_name='A1',values=cd)
    print(f"   ✅ {len(cd)-1}행")
    ...
    for p in PRODUCTS: bd.append([f'{p} 예산']+[round(daily_product.get((d,p),{}).get('spend',0)) for d in dc])
    ws_b.clear(); ws_b.update(range_name='A1',values=bd)
    print(f"   ✅ {len(bd)}행"); time.sleep(2)
    ...
    wp = OrderedDict()
    for date in all_dates:
        dt=date_key_to_dt(date); s=dt-timedelta(days=dt.weekday()); e=s+timedelta(days=6)
        wp.setdefault(f"{s.month}/{s.day}~{e.month}/{e.day}",[]).append(date)
    wks=list(wp.keys())
    wt={}
    for wk,diw in wp.items():
        sp=sum(daily_total.get(d,{}).get('spend',0) for d in diw); rv=sum(daily_total.get(d,{}).get('revenue',0) for d in diw)
    ...
    for r in master_rows:
        dt=date_key_to_dt(r['date']); s=dt-timedelta(days=dt.weekday()); e=s+timedelta(days=6)
        wk=f"{s.month}/{s.day}~{e.month}/{e.day}"
        wa[r['adset_id']][wk]['spend']+=r['spend']; wa[r['adset_id']][wk]['revenue']+=r['revenue_mp']
    wtd=[['캠페인 이름','광고 세트 이름','광고 세트 ID','전체 평균']+wks]
    for cn,an,aid in tao:
        if cn=='종합':
            row=['종합','','']; ts=sum(w['spend'] for w in wt.values()); tr=sum(w['revenue'] for w in wt.values())
            row.append(trend_cell(ts,tr,tr-ts,tr/ts*100 if ts>0 else 0))
            for wk in wks: w=wt.get(wk,{}); row.append(trend_cell(w.get('spend',0),w.get('revenue',0),w.get('profit',0),w.get('roas',0)))
        else:
            row=[cn or '',an or '',aid or '']; wad=wa.get(aid,{})
            ts=sum(w['spend'] for w in wad.values()); tr=sum(w['revenue'] for w in wad.values())
            row.append(trend_cell(ts,tr,tr-ts,tr/ts*100 if ts>0 else 0))
            for wk in wks:
                w=wad.get(wk,{'spend':0,'revenue':0}); s,r_=w['spend'],w['revenue']
                row.append(trend_cell(s,r_,r_-s,r_/s*100 if s>0 else 0) if s>0 or r_>0 else '')
        wtd.append(row)
    ws_wt.clear(); ws_wt.update(range_name='A1',values=wtd)
    print(f"   ✅ {len(wtd)-1}행")
    ...
    for r in master_rows:
        dt=date_key_to_dt(r['date']); s=dt-timedelta(days=dt.weekday()); e=s+timedelta(days=6)
        wk=f"{s.month}/{s.day}~{e.month}/{e.day}"
        ld=calendar.monthrange(dt.year,dt.month)[1]; mk=f"{dt.month}/1~{dt.month}/{ld}"; p=r['product']
        wprod[wk][p]['spend']+=r['spend']; wprod[wk][p]['revenue']+=r['revenue_mp']; wprod[wk][p]['pm']+=r['purchases_mp']
        mprod[mk][p]['spend']+=r['spend']; mprod[mk][p]['revenue']+=r['revenue_mp']; mprod[mk][p]['pm']+=r['purchases_mp']
    wtf={}
    for wk,diw in wp.items():
        sp=sum(daily_total.get(d,{}).get('spend',0) for d in diw); rv=sum(daily_total.get(d,{}).get('revenue',0) for d in diw)
        pm=sum(daily_total.get(d,{}).get('purchases_meta',0) for d in diw); pmp=sum(daily_total.get(d,{}).get('purchases_mp',0) for d in diw)
        uc=sum(sum(r['unique_ob_clicks'] for r in master_rows if r['date']==d) for d in diw)
        wtf[wk]={'spend':sp,'revenue':rv,'profit':rv-sp,'roas':rv/sp*100 if sp>0 else 0,'purchases_meta':pm,'purchases_mp':pmp,'cvr':pmp/uc*100 if uc>0 else 0}
    mps=OrderedDict()
    for date in all_dates:
        dt=date_key_to_dt(date); ld=calendar.monthrange(dt.year,dt.month)[1]; mps.setdefault(f"{dt.month}/1~{dt.month}/{ld}",[]).append(date)
    mt={}
    for mk,dim in mps.items():
        sp=sum(daily_total.get(d,{}).get('spend',0) for d in dim); rv=sum(daily_total.get(d,{}).get('revenue',0) for d in dim)
        pm=sum(daily_total.get(d,{}).get('purchases_meta',0) for d in dim); pmp=sum(daily_total.get(d,{}).get('purchases_mp',0) for d in dim)
        uc=sum(sum(r['unique_ob_clicks'] for r in master_rows if r['date']==d) for d in dim)
        mt[mk]={'spend':sp,'revenue':rv,'profit':rv-sp,'roas':rv/sp*100 if sp>0 else 0,'purchases_meta':pm,'purchases_mp':pmp,'cvr':pmp/uc*100 if uc>0 else 0}
    def bwb(pk,pd,prd):
        b=[[pk]+['']*10,['지출 금액','구매 (메타)','구매 (믹스패널)','매출','이익','ROAS','CVR','전체','','',''],
           [fmt(pd.get('spend',0)),fmt(pd.get('purchases_meta',0)),fmt(pd.get('purchases_mp',0)),
            fmt(pd.get('revenue',0)),fmt(pd.get('profit',0)),fmtp(pd.get('roas',0)),fmtp(pd.get('cvr',0)),'','','',''],
           ['']+PRODUCTS+['합']]
        for lbl,fn,tk in [('제품별 ROAS',lambda d:fmtp(d['revenue']/d['spend']*100 if d['spend']>0 else 0),'roas'),
                          ('제품별 순이익',lambda d:fmt(d['revenue']-d['spend']),'profit'),
                          ('제품별 매출',lambda d:fmt(d['revenue']),'revenue'),
                          ('제품별 예산',lambda d:fmt(d['spend']),'spend')]:
            row=[lbl]
            for p in PRODUCTS: row.append(fn(prd.get(p,{'spend':0,'revenue':0})))
            row.append(fmtp(pd.get(tk,0)) if tk=='roas' else fmt(pd.get(tk,0))); b.append(row)
        sr=['제품별 예산 비중']; tsp=pd.get('spend',0)
        for p in PRODUCTS: sr.append(fmtp(prd.get(p,{'spend':0})['spend']/tsp*100 if tsp>0 else 0))
        sr.append('100.00%'); b.append(sr); b.append(['']*11); return b

    wsd=[]
    for mk in mps.keys():
        wsd.extend(bwb(mk,mt[mk],{p:mprod[mk].get(p,{'spend':0,'revenue':0,'pm':0}) for p in PRODUCTS}))
        for wk in wks:
            if any(d in set(mps.get(mk,[])) for d in wp.get(wk,[])): wsd.extend(bwb(wk,wtf[wk],{p:wprod[wk].get(p,{'spend':0,'revenue':0,'pm':0}) for p in PRODUCTS}))
    sh.worksheet('주간종합').clear(); sh.worksheet('주간종합').update(range_name='A1',values=wsd)
    print(f"   ✅ 주간종합: {len(wsd)}행"); time.sleep(2)

    ws2=sh.worksheet('주간종합_2')
    w2=[['📊 기간별 전체 요약','','','','','',''],['기간','유형','지출금액','매출','이익','ROAS','CVR']]
    for mk in mps.keys():
        m=mt[mk]; w2.append([mk,'월별',fmt(m['spend']),fmt(m['revenue']),fmt(m['profit']),fmtp(m['roas']),fmtp(m['cvr'])])
        for wk in wks:
            if any(d in set(mps.get(mk,[])) for d in wp.get(wk,[])):
                w=wtf[wk]; w2.append([wk,'주간',fmt(w['spend']),fmt(w['revenue']),fmt(w['profit']),fmtp(w['roas']),fmtp(w['cvr'])])
    w2.extend([[],[],['📈 제품별 ROAS','','','','','',''],['기간','유형']+PRODUCTS])
    for mk in mps.keys():
        row=[mk,'월별']
        for p in PRODUCTS: d=mprod[mk].get(p,{'spend':0,'revenue':0}); row.append(fmtp(d['revenue']/d['spend']*100 if d['spend']>0 else 0))
        w2.append(row)
        for wk in wks:
            if any(d in set(mps.get(mk,[])) for d in wp.get(wk,[])):
                row=[wk,'주간']
                for p in PRODUCTS: d=wprod[wk].get(p,{'spend':0,'revenue':0}); row.append(fmtp(d['revenue']/d['spend']*100 if d['spend']>0 else 0))
                w2.append(row)
    ws2.clear(); ws2.update(range_name='A1',values=w2)
    print(f"   ✅ 주간종합_2: {len(w2)}행"); time.sleep(2)

    # --- 5g. 주간종합_3 ---
    print(f"\n   [7/7] 주간종합_3...")
    ws3=sh.worksheet('주간종합_3')
    w3=[['📊 일별 전체 요약','','','','','','',''],['날짜','요일','지출금액','매출','이익','ROAS','CVR','']]
    for date in all_dates:
        dt=daily_total.get(date,{})
        w3.append([date,date_key_to_weekday(date),fmt(dt.get('spend',0)),fmt(dt.get('revenue',0)),fmt(dt.get('profit',0)),fmtp(dt.get('roas',0)),fmtp(dt.get('cvr',0)),''])
    for lbl,vk in [('ROAS','roas'),('순이익','profit'),('매출','revenue'),('예산','spend')]:
        w3.append([]); w3.append(['날짜','요일']+PRODUCTS)
        for date in all_dates:
            row=[date,date_key_to_weekday(date)]
            for p in PRODUCTS: v=daily_product.get((date,p),{}).get(vk,0); row.append(fmtp(v) if vk=='roas' else fmt(v))
            w3.append(row)
    w3.append(['📊 일별 제품별 예산 비중','','','','','','',''])
    w3.append(['날짜','요일']+PRODUCTS)
    for date in all_dates:
        row=[date,date_key_to_weekday(date)]; tsp=daily_total.get(date,{}).get('spend',0)
        for p in PRODUCTS: row.append(fmtp(daily_product.get((date,p),{}).get('spend',0)/tsp*100 if tsp>0 else 0))
        w3.append(row)
    ws3.clear(); ws3.update(range_name='A1',values=w3)
    print(f"   ✅ {len(w3)}행")
...
# =============================================================================
def main():
    meta_data, all_adsets = fetch_meta()
    mp_matched, total_events, matched_events, _ = fetch_mixpanel(all_adsets)
    ...
    n_rows, n_adsets = update_sheets(master_rows, all_dates, all_adsets, daily_total, daily_product, adset_daily)

    print(f"\n{'='*80}")
    print(f"✅ 전체 업데이트 완료!")
    print(f"   Meta: {META_AD_ACCOUNT_ID} | 광고세트: {n_adsets}개")
    print(f"   Mixpanel: {total_events}건 | 매칭 {matched_events}건 ({matched_events/max(total_events,1)*100:.1f}%)")
    print(f"   기간: {FROM_DATE} ~ {TO_DATE} ({len(all_dates)}일)")
    print(f"{'='*80}")
