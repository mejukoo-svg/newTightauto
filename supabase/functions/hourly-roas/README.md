...
## 주별·월별 셀 (2026-08-27 추가)

추이차트 '보기'를 **주별/월별**로 바꿔도 셀을 누르면 같은 화면이 열린다. 버킷 하나가 여러 날이므로
**그 구간의 모든 날을 시각(0~23시)으로 접어서 합산**한다 — "이 주(달)에는 몇 시가 잘 나왔나" 를
보는 화면이지 날짜별 추이가 아니다(그건 추이차트 본체가 이미 보여 준다).

- 요청: `{ mode, date_from, date_to, adset_id | sets[] }` (하루짜리는 종전대로 `date` 도 보낸다)
- 응답에 `date_from` / `date_to` / `days` 가 추가된다. 화면은 KPI·표에 **일평균**을 같이 띄운다.
- **구간의 양 끝은 화면에 실제로 있는 날짜**다. 맨 왼쪽 주/월은 진행 중이고 맨 오른쪽은 기간
  컷오프로 잘려 있어 달력상의 주/월과 다르다 — 달력 기준으로 보내면 화면에 없는 날이 섞여
  셀 값과 안 맞는다.
- 한 번에 **31일**까지(`MAX_SPAN_DAYS`). 더 길면 Mixpanel export 수신만으로 실행시간을 넘긴다.

구현상 주의 두 가지:

1. **Mixpanel export 는 스트리밍으로 파싱한다.** `res.text()` 로 통째로 받으면 한 달치(20만 줄
   이상)에서 메모리가 터진다. `mpFetchLines()` 가 줄 단위로 흘려보내며 필요한 필드만 남긴다.
2. **구간이면 Meta `time_increment=all_days`.** `"1"` 이면 응답이 (세트 × 날짜 × 24시각) 행이라
   종합 행(세트 수백 개) × 한 달에서 7만 행을 넘겨 페이지 상한에 걸려 **조용히 잘린다**.
