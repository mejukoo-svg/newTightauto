...
3. 원본 광고를 메타에서 읽어 소속 계정 대조 + **캠페인명에서 상품명 추출**
   (`국내_소재별_supabase.py` 의 `extract_product` 와 같은 규칙: 구분자로 쪼갠 첫 비숫자 토큰, 앞 이모지 제거)
4. 같은 계정의 캠페인 중 이름에 `ASC` 가 들어가고 상품명이 같은 것 → 하위 세트 전부가 대상
