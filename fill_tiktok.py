...
ROWS = [
    ...
    # --- 2026-09-12 신규 2개 (09/12 첫 지출 · ABO 500,000) --- 둘 다 형제 없는 신규 캠페인이라
    # 대용키(= 자기 광고그룹 ID). 09/12 캠페인 레벨 리포트엔 Campaign ID 컬럼이 없었다(20~21컬럼판).
    # 무녀re 는 09/12 utm_id 로 37,500원 1건이 직접 왔다(ROWS 추가 전엔 미매칭으로 떴음).
    ("무녀_260912_무녀re", "1876119020197969", "1876119020197969"),
    ("무당_260912_모든소재_re", "1876116524043681", "1876116524043681"),
    # --- 2026-09-15 신규 2개 (09/15 첫 지출 6,263·601 · ABO 500,000 · 입찰 20,000) --- 둘 다 형제 없는
    # 신규 캠페인이라 대용키(= 자기 광고그룹 ID). 09/14·09/15 리포트 모두 Campaign ID 컬럼 없는 32컬럼판.
    ("무당_260915_숏폼드라마모음", "1876382909148353", "1876382909148353"),
    ("무당_260915_인플모음", "1876383405076609", "1876383405076609"),
]
...
CAMP_NAME = {
    ...
    # 2026-09-12 신규 캠페인 2개. 키는 대용키(첫 광고그룹 ID) — 캠페인 레벨 리포트로 교체 필요.
    "1876119020197969": "무녀_260912_무녀re",
    "1876116524043681": "무당_260912_모든소재_re",
    # 2026-09-15 신규 캠페인 2개. 키는 대용키(첫 광고그룹 ID) — 캠페인 레벨 리포트로 교체 필요.
    "1876382909148353": "무당_260915_숏폼드라마모음",
    "1876383405076609": "무당_260915_인플모음",
}
...
def load_revenue(dates, spend):
    ...
    # ⚠️ 같은 결제의 중복 이벤트가 **같은 $insert_id 인데 프로퍼티가 다르게** 온다(2026-09-12 실측:
    # order_1789167397735 은 5건 중 2건이 utm_id 없는 서버 이벤트). export 줄 순서가 매번 달라서
    # "먼저 온 것"을 남기면 utm 없는 쪽이 살아남아 결제가 조용히 미매칭으로 빠진다(49,800원 1건이
    # 실행마다 있다/없다 했다). 그래서 order_id(없으면 $insert_id)로 묶어 **utm 이 가장 온전한
    # 이벤트를 대표로** 고른 뒤 종전 로직을 돌린다.
    groups = {}
    for l in mp_fetch(cfg, lo, hi):
        p = json.loads(l)["properties"]
        key = ("o", p["order_id"]) if p.get("order_id") is not None else ("i", p.get("$insert_id") or id(p))
        rank = (bool(p.get("utm_id")), str(p.get("utm_source") or "").lower() == SRC, bool(p.get("utm_content")))
        if key not in groups or rank > groups[key][0]:
            groups[key] = (rank, p)
    for _, p in groups.values():
        ins = p.get("$insert_id")
        if ins:
            if ins in seen_ins:
                continue
