...
def main():
    ...
    # =======================================================
    # 8단계: 토스페이먼츠 일별 매출 (국내_매출.py 로직)
    # =======================================================
    if TOSS_SECRET_KEY:
        log.info(f"\n8단계: 토스페이먼츠 일별 매출")
        toss_start = DATA_REFRESH_START.strftime("%Y-%m-%d")
        toss_end = TODAY.strftime("%Y-%m-%d")
        log.info(f"  📅 기간: {toss_start} ~ {toss_end}")

        auth_string = base64.b64encode(f"{TOSS_SECRET_KEY}:".encode()).decode()
        toss_headers = {"Authorization": f"Basic {auth_string}", "Content-Type": "application/json"}
        REVENUE_STATUSES = {"DONE", "PARTIAL_CANCELED"}
        toss_records = []
        current = DATA_REFRESH_START

        while current <= TODAY:
            ds = current.strftime("%Y-%m-%d")
            params = {"startDate": f"{ds}T00:00:00+09:00", "endDate": f"{ds}T23:59:59+09:00", "limit": 100}
            total_amt, total_cnt, cancel_amt, cancel_cnt = 0, 0, 0, 0

            while True:
                try:
                    resp = req_lib.get("https://api.tosspayments.com/v1/transactions", headers=toss_headers, params=params, timeout=30)
                    if resp.status_code != 200: break
                    data = resp.json()
                    if isinstance(data, list):
                        txns = data; has_more = len(data) >= 100; cursor = data[-1].get('transactionKey') if data else None
                    elif isinstance(data, dict):
                        txns = data.get('data', data.get('transactions', [])); has_more = data.get('hasMore', False)
                        cursor = data.get('lastCursor') or (txns[-1].get('transactionKey') if txns else None)
                    else: break
                    if not txns: break
                    for txn in txns:
                        status = txn.get("status", ""); amt = txn.get("amount", 0) or 0
                        if status in REVENUE_STATUSES: total_amt += amt; total_cnt += 1
                        elif status == "CANCELED": cancel_amt += abs(amt); cancel_cnt += 1
                    if (has_more or len(txns) >= 100) and cursor:
                        params = {"startDate": f"{ds}T00:00:00+09:00", "endDate": f"{ds}T23:59:59+09:00", "limit": 100, "startingAfter": cursor}
                        time.sleep(0.2)
                    else: break
                except Exception as e:
                    log.error(f"  ❌ 토스 오류 ({ds}): {e}"); break

            net = total_amt - cancel_amt
            toss_records.append({"date": ds, "total_amount": total_amt, "total_count": total_cnt,
                "cancel_amount": cancel_amt, "cancel_count": cancel_cnt, "net_amount": net, "net_count": total_cnt - cancel_cnt})
            log.info(f"  📅 {ds}: ₩{total_amt:,.0f} ({total_cnt}건) | 순 ₩{net:,.0f}")
            current += timedelta(days=1); time.sleep(0.3)

        if toss_records:
            sb.upsert("toss_daily_revenue", toss_records, chunk_size=100)
        log.info(f"✅ 토스: {len(toss_records)}일, 합계 ₩{sum(r['total_amount'] for r in toss_records):,.0f}")
    else:
        log.info("\n⏭️ TOSS_SECRET_KEY 없음 → 토스 매출 스킵")
