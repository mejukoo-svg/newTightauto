...
PRODUCT_KEYWORDS = ["퀴어", "집착", "결혼", "커리어", "재물", "솔로", "재회", "29금궁합", "자녀", "29금", "1%", "별해"]
...
SUMMARY_PRODUCTS = ["퀴어", "집착", "결혼", "커리어", "1%", "별해", "솔로", "재물", "재회", "29금궁합", "자녀", "29금"]
...
def move_analysis_tab(sh, ws, tab_name):
    ...
    except Exception as e: print(f"  ⚠️ '{tab_name}' 이동 오류: {e}")
...
def reorder_tabs(sh):
    try:
        ...
        reorder_ok = False
        try:
            sh.reorder_worksheets(final_order); reorder_ok = True; print("  ✅ reorder_worksheets 성공")
        except AttributeError: print("  ⚠️ reorder_worksheets 미지원 → 개별 이동 방식으로 전환")
        except Exception as e1: print(f"  ⚠️ reorder_worksheets 실패: {e1} → 개별 이동 방식으로 전환")
        if not reorder_ok:
            ...
            for idx, ws in enumerate(final_order):
                try:
                    ...
                    if idx < 5 or idx == len(final_order)-1: print(f"    [{idx}] {ws.title}")
                    time.sleep(0.5)
                except Exception as e2: print(f"    ⚠️ [{idx}] {ws.title} 이동 실패: {e2}"); time.sleep(2)
            print("  ✅ 개별 이동 완료")
        time.sleep(2)
...
def apply_trend_chart_formatting(sh, ws, headers, rows_count, is_change_tab=False, sunday_col_indices=None, format_col_end=None):
    ...
    # ★ FIX v14: 괄호 안 ROAS 추출 수식으로 변경
    print("  🎨 ROAS 조건부 서식...")
    try:
        sc, ec = 3, len(headers)
        _V = 'VALUE(MID(INDIRECT(ADDRESS(ROW(),COLUMN())),FIND("(",INDIRECT(ADDRESS(ROW(),COLUMN())))+1,FIND(")",INDIRECT(ADDRESS(ROW(),COLUMN())))-FIND("(",INDIRECT(ADDRESS(ROW(),COLUMN())))-1))'
        _PRE = '=AND(NOT(ISBLANK(INDIRECT(ADDRESS(ROW(),COLUMN())))),LEN(TRIM(INDIRECT(ADDRESS(ROW(),COLUMN()))))>0,'
        rules = [
            {'c':f'{_PRE}{_V}=0)','clr':{'red':1.0,'green':0.6,'blue':0.6}},
            {'c':f'{_PRE}{_V}>=300)','clr':{'red':0.6,'green':1.0,'blue':1.0}},
            {'c':f'{_PRE}{_V}>=200,{_V}<300)','clr':{'red':0.7,'green':1.0,'blue':0.7}},
            {'c':f'{_PRE}{_V}>=100,{_V}<200)','clr':{'red':1.0,'green':1.0,'blue':0.6}},
            {'c':f'{_PRE}{_V}<100,{_V}>0)','clr':{'red':1.0,'green':0.8,'blue':0.8}},
        ]
...
def generate_date_tab_summary(rows, structure="new"):
    ...
    for table_title, data_type in tables:
        ...
        if data_type=="roas": r_pv[9+len(sp)]=total_prod_revenue/total_prod_spend if total_prod_spend>0 else 0
        elif data_type=="profit": r_pv[9+len(sp)]=round(total_prod_profit)
        elif data_type=="revenue": r_pv[9+len(sp)]=round(total_prod_revenue)
        elif data_type=="profit_margin": r_pv[9+len(sp)]=total_prod_profit/total_prod_revenue if total_prod_revenue>0 else 0
        elif data_type=="spend": r_pv[9+len(sp)]=round(total_prod_spend)
        elif data_type=="spend_ratio": r_pv[9+len(sp)]=""
        elif data_type=="cvr": total_pc=sum(prod_clicks[p] for p in sp);total_pm=sum(prod_mpc[p] for p in sp);r_pv[9+len(sp)]=total_pm/total_pc if total_pc>0 else 0
...
print("="*60); print(f"2단계: Meta Insights 수집 (adset 레벨, 최근 {REFRESH_DAYS}일) [★ 병렬]"); print("="*60)
meta_date_data = defaultdict(list); meta_success_count = 0

# ★ 병렬: (날짜, 계정) 조합을 모두 동시에 호출
def _fetch_one_meta(day_offset, acc_id):
    """하나의 (날짜, 계정) 조합을 가져오는 워커"""
...
tasks = [(day_offset, acc_id) for day_offset in range(META_COLLECT_DAYS) for acc_id in ALL_AD_ACCOUNTS]
print(f"  📡 총 {len(tasks)}개 작업 (날짜 {META_COLLECT_DAYS} × 계정 {len(ALL_AD_ACCOUNTS)}) 병렬 실행...")

with ThreadPoolExecutor(max_workers=6) as executor:
    futures = {executor.submit(_fetch_one_meta, do, ai): (do, ai) for do, ai in tasks}
    for future in as_completed(futures):
        try:
            date_key, target_date, acc_id, parsed = future.result()
            if parsed:
                meta_date_data[date_key].extend(parsed)
                meta_success_count += len(parsed)
                print(f"  ✅ {date_key} / {acc_id}: {len(parsed)}건")
            else:
                print(f"  ⚠️ {date_key} / {acc_id}: 0건")
        except Exception as e:
            do, ai = futures[future]
            print(f"  ❌ day_offset={do}, acc={ai} 오류: {e}")

print(f"\n✅ Meta 수집 완료 (병렬): {len(meta_date_data)}일, 총 {meta_success_count}건\n")

print("="*60); print("2.5단계: 광고 세트별 일일 예산 조회 [★ 병렬]"); print("="*60)
adset_budget_map = {}

def _fetch_one_budget(acc_id):
    return acc_id, fetch_adset_budgets(acc_id)

with ThreadPoolExecutor(max_workers=3) as executor:
    futures = {executor.submit(_fetch_one_budget, ai): ai for ai in ALL_AD_ACCOUNTS}
    for future in as_completed(futures):
        try:
            acc_id, budgets = future.result()
            adset_budget_map.update(budgets)
            print(f"  ✅ {acc_id}: {len(budgets)}개 활성 세트 예산 조회")
        except Exception as e:
            print(f"  ❌ 예산 조회 오류: {e}")

print(f"\n✅ 전체 예산 조회 완료 (병렬): {len(adset_budget_map)}개 세트")
if adset_budget_map:
...
print(f"\n  ── Part 1: 어제까지 청크 호출 [★ 병렬] ──")
# 청크 목록 미리 생성
mp_chunks = []
while current_start <= YESTERDAY:
    current_end = min(current_start + timedelta(days=MIXPANEL_CHUNK_DAYS-1), YESTERDAY)
    chunk_from = current_start.strftime('%Y-%m-%d'); chunk_to = current_end.strftime('%Y-%m-%d')
    chunk_idx += 1
    mp_chunks.append((chunk_idx, chunk_from, chunk_to))
    current_start = current_end + timedelta(days=1)
# 오늘도 청크에 추가
mp_chunks.append((chunk_idx + 1, mp_to_today, mp_to_today))
print(f"  📦 총 {len(mp_chunks)}개 청크 병렬 실행...")

def _fetch_one_mixpanel(idx, from_d, to_d):
    return idx, fetch_mixpanel_data(from_d, to_d)

with ThreadPoolExecutor(max_workers=4) as executor:
    futures = {executor.submit(_fetch_one_mixpanel, ci, cf, ct): ci for ci, cf, ct in mp_chunks}
    for future in as_completed(futures):
        try:
            idx, chunk_data = future.result()
            mp_raw.extend(chunk_data)
            print(f"  ✅ 청크 {idx}: {len(chunk_data)}건 (누적: {len(mp_raw)}건)")
        except Exception as e:
            print(f"  ❌ Mixpanel 청크 오류: {e}")
...
# ★ FIX v14: 증감액 cell_text_change에 revenue= 추가
print("\n12단계: 증감액")
...
src.append(cell_text_change(t7roas,sdc,t7s,(t7cs/t7cc) if t7cc>0 else 0,(t7vs/t7vc) if t7vc>0 else 0,revenue=t7r))
for i,d in enumerate(chart_dn):
    ...
    src.append(cell_text_change(d_roas,chg,ds_v,(dcs/dcc) if dcc>0 else 0,(dvs/dvc) if dvc>0 else 0,revenue=dr))
rtc=[]
for it in sorted_list:
    ...
    r.append(cell_text_change(ar,cp,ts_v,(tcpm/tcpc) if tcpc>0 else 0,(tcvr/tcvc) if tcvc>0 else 0,revenue=tr_v))
    for i,d in enumerate(chart_dn):
        if d in it['data']['dates']:
            ...
            r.append(cell_text_change(roas,chg,dt['spend'],dt.get('cpm',0),dt.get('cvr',0),revenue=dt['revenue']))
        else:r.append('')
    rtc.append(r)
